...
//нижняя таблица детализации
   var strRes = '';
  /*  strRes += '<br><hr><h3>Детализация</h3><br><table class="result-table">';
    strRes += '<thead>';
    strRes += '<tr>';
    strRes += '<td>ID</td>';
    strRes += '<td>Пользователь</td>';
    strRes += '<td>Внешний номер</td>';
    strRes += '<td>Направление звонка</td>';
    strRes += '<td>Длительность разговора</td>';
    strRes += '<td>Дата звонка</td>';
    strRes += '</tr>';
    strRes += '</thead>';
    strRes += '<tbody>';*/
...
//убрать вывод нижней таблицы
/*for(var k in data){
        arUser = AppCS.getUserByID(data[k].PORTAL_USER_ID);

        if (AppCS.userPhoneSet.has(AppCS.normalizeNumber(data[k].PHONE_NUMBER))) {
            // console.log("skipping " + data[k].PHONE_NUMBER + " " + arUser.LAST_NAME+ ' ' + arUser.NAME + (new Date(data[k].CALL_START_DATE)).format("dd.mm.yyyy HH:MM"));
            continue;
        }

        uniqueNumbers.add(data[k].PHONE_NUMBER);

        sumData.CALLS_COUNT++;
        sumData.DURATION_SUM += parseInt(data[k].CALL_DURATION);
        arUser = AppCS.getUserByID(data[k].PORTAL_USER_ID)
        if(typeof arUser == 'undefined') continue;
        //if(typeof arUser.LAST_NAME == 'undefined') console.log(arUser);;
        var type = 'не определено';
        if(data[k].CALL_TYPE == 2){
            sumData.IN_COUNT++;
            type = 'Входящий';
        }
        if(data[k].CALL_TYPE == 1){
            sumData.OUT_COUNT++;
            type = 'Исходящий';
        }
        strRes+= '<tr>';
        strRes+= '<td>'+data[k].ID+'</td>';
        strRes+= '<td>('+data[k].PORTAL_USER_ID+') '+arUser.LAST_NAME+' '+arUser.NAME+'</td>';
        strRes+= '<td>'+data[k].PHONE_NUMBER+'</td>';
        strRes+= '<td>'+type+'</td>';
        var t = getBeautyStrTime(data[k].CALL_DURATION);
        strRes += '<td>'+t.H+':'+t.M+':'+t.S+'</td>';
        strRes+= '<td>'+(new Date(data[k].CALL_START_DATE)).format("dd.mm.yyyy HH:MM")+'</td>';
        strRes+= '</tr>';
    }*/
    strRes += '</tbody>';
    strRes += '</table>';
    ...
    var avgRes = '';
    avgRes += '<div class="avg-result">';
    avgRes += '<div>Уникальных внешних номеров - '+ uniqueNumbers.size +'</div>';
    avgRes += '<div>Всего звонков - '+sumData.CALLS_COUNT+'</div>';
    avgRes += '<div>Всего входящих - '+sumData.IN_COUNT+'</div>';
    avgRes += '<div>Всего исходящих - '+sumData.OUT_COUNT+'</div>';
    var t = getBeautyStrTime(sumData.DURATION_SUM);
    var strT = t.H+':'+t.M+':'+t.S;
    avgRes += '<div>Общее время - '+strT+'</div>';
    var t = getBeautyStrTime((sumData.DURATION_SUM / sumData.CALLS_COUNT).toFixed(0));
    var strT = t.H+':'+t.M+':'+t.S;
    avgRes += '<div>Среднее время разговора - '+strT+'</div>';
    avgRes += '</div>';
    ...
    for(var k in data){
        ...
        if(typeof avgData['user_'+data[k].PORTAL_USER_ID] == "undefined"){
            avgData['user_'+data[k].PORTAL_USER_ID] = {
        ...
        avgData['user_'+data[k].PORTAL_USER_ID]['UNIQUE_EXTERNAL_NUMBERS'].add(data[k].PHONE_NUMBER)
        avgData['user_'+data[k].PORTAL_USER_ID]['DURATION_SUM'] += parseInt(data[k]['CALL_DURATION']);

        if (data[k].REST_APP_NAME=="Виртуальная АТС от МегаФон"){
            avgData['user_'+data[k].PORTAL_USER_ID]['DURATION_CELL_SUM'] += parseInt(data[k]['CALL_DURATION']);
        }

        avgData['user_'+data[k].PORTAL_USER_ID]['CALLS_COUNT']++;
        if(data[k].CALL_TYPE == 2){
            avgData['user_'+data[k].PORTAL_USER_ID]['IN_COUNT']++;
        }
        if(data[k].CALL_TYPE == 1){
            avgData['user_'+data[k].PORTAL_USER_ID]['OUT_COUNT']++;
        }
    }
    ...
    strAvg += '<th>Уникальных внешних номеров</th>';
    strAvg += '<th>Всего звонков</th>';
    strAvg += '<th>Исходящих</th>';
    strAvg += '<th>Входящих</th>';
    strAvg += '<th>Общее время</th>';
    strAvg += '<th>Общее время мобильные</th>';
    strAvg += '<th>Среднее время разговора</th>';
    strAvg += '</tr>';
    strAvg += '</thead>';
    strAvg += '<tbody>';
    for(var k in avgData){
        ...
        strAvg += '<tr>';
            ...
            //убрала фото
              /* if(user.PERSONAL_PHOTO != null){
                    strAvg += '<img alert="'+user.LAST_NAME+'" class="user-photo" src="'+user.PERSONAL_PHOTO+'" />';
                }else{
                    strAvg += '<img alert="'+user.LAST_NAME+'" class="user-photo" src="img/user.jpg" />';
        ...
        strAvg += '<td>'+avgData[k].OUT_COUNT+'</td>';
        strAvg += '<td>'+avgData[k].IN_COUNT+'</td>';
        var t = getBeautyStrTime(avgData[k].DURATION_SUM);
        strAvg += '<td>'+t.H+':'+t.M+':'+t.S+'</td>';

        var t = getBeautyStrTime(avgData[k].DURATION_CELL_SUM);
        strAvg += '<td>'+t.H+':'+t.M+':'+t.S+'</td>';

        var t = getBeautyStrTime(avgData[k].DURATION_AVG);
        strAvg += '<td>'+t.H+':'+t.M+':'+t.S+'</td>';
        strAvg += '</tr>';
    }
